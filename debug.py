...
async def main():
    async with async_playwright() as p:
        ...
        await page.set_viewport_size({"width": 200, "height": 150})  # Ukuran viewport lebih besar
        await page.goto("https://tokopedia.com/beli-langsung", wait_until='domcontentloaded')

        try:
            await page.wait_for_selector('div[data-testid="lblOSPSummaryTotalHarga"] p', timeout=5000)
        except Exception as e:
            print(f"Error: {str(e)}")
            await browser.close()
            return
